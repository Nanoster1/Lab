...
namespace Lab05_Factorial
{
    class Program
    {
        ...
        static void IsIntoOutput(string conc)                                              //Табличка ("Внутренности вывода")
        {
            Console.Clear();
            SetCursor(conc, -1);
            IsDoubleIntoOut(conc, '┌', '┐');
            SetCursor(conc, 0);
            Console.Write(conc);
            SetCursor(conc, 1);
            IsDoubleIntoOut(conc, '└', '┘');
        ...
        static void IsDoubleIntoOut(string conc, char leftSymbol, char rightSymbol)        //строки рамки                  
        {
        ...
        static ulong Сalculate(ulong number)                                               //Вычисление факториала
        {       
            ulong fact = 1;
